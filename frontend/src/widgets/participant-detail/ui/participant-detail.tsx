...
import {Award, MessageCircle, User} from "lucide-react";
...
/**
 * TwitterIDから@を除去してユーザー名のみを取得
 */
function normalizeTwitterId(twitterId: string): string {
    return twitterId.startsWith('@') ? twitterId.slice(1) : twitterId;
}
...
export function ParticipantDetail({participant, isOpen, onClose}: ParticipantDetailProps) {
    ...
    const normalizedTwitterId = normalizeTwitterId(participant.twitterId);
    return (
        <Dialog open={isOpen} onOpenChange={onClose}>
            <DialogContent className="max-w-md bg-gradient-to-br from-cyan-50 to-blue-50 max-h-[90vh] overflow-y-auto">
                ...
                <div className="space-y-4 mt-4">
                    ...
                    <Card className="p-3 bg-white border-cyan-200">
                    ...
                    <Card className="p-3 bg-white border-cyan-200">
                        <div className="flex items-start gap-2">
                            ...
                            <div>
                                ...
                                <a
                                    href={`https://twitter.com/${normalizedTwitterId}`}
                                    target="_blank"
                            ...
                            {participant.stamps.map((stamp) => (
                                <Card
                                    ...
                                    className={`p-3 transition-all ${
                                        stamp.isCollected ? "bg-cyan-100 border-cyan-300" : "bg-white border-gray-200"
                                    }`}
                                >
                                    <div className="flex items-center gap-3">
                                        <StampImage
                                            stampName={stamp.name}
                                            isAcquired={stamp.isCollected}
                                            size="small"
                                        />
